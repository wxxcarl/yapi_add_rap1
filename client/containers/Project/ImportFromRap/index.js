...
class Activity extends Component {
  ...
  formatDeep(key) {
    ...
    key.forEach(rp => {
      ...
      if(rp.dataType.match(/array<(.*)>/)) {
        ...
        let mock = rp.remark ? rp.remark.replace('@mock=','') : ''
        ...
        // res_body.properties[identifier] = rp.parameterList.length === 0 ? {
        //   description: rp.name,
        //   mock: remark ? {
        //     mock: remark
        //   } : undefined,
        ...
        //   minLength: len ? len.split('-')[0] : undefined,
        //   maxLength: len ? (len.split('-')[1] ? len.split('-')[1] : len.split('-')[0]) : undefined
        // } : this.formatDeep(rp.parameterList)
      }
    })
    return res_body
  }
  ...
  async requestRap(id) {
    ...
    console.log('rap-data=>', res.data.data)
    if(res.data.data){
      message.info(`远程获取RAP数据成功`);
    }
    let folder = res.data.data.name
    let catid
    if(folderList.includes(folder)===false){
      await axios.post('/api/interface/add_cat', {
        desc: folder,
        name: folder,
        project_id: this.props.match.params.id
      }).then(res2 => {
        message.info(`新增接口分类[${folder}]成功`);
        catid = res2.data.data._id
      })
    }
    if(!catid){
      message.error(`创建接口分类[${folder}]失败`);
      return false
    }
    res.data.data.moduleList.forEach( e=> {
      e.pageList.forEach(j=>{
        j.actionList.forEach( t => {
          if(t.requestUrl){
            ...
            axios.post('/api/interface/add',createParams).then(res3 => {
              console.log(`插入接口[${t.name}]模板成功`)
              if(rm != 'GET' && rm != 'POST'){
                message.error(`【${rm}】请求方式导入可能有问题`);
              }
              ...
              if(rm === 'GET') {
                t.requestParameterList.forEach( rp => {
                  req_query.push({
                    desc: rp.name,
                    example: rp.remark.replace('@mock=',''),
                    name: rp.identifier,
                    required: "1"
                  })
                })
              ...
              axios.post('/api/interface/up', upparams).then(() => {
                message.info(`插入接口[${t.name}]成功`);
              })
            })
          }
        })
